#include "c:/Program Files (x86)/Common Files/Adobe/CEP/extensions/com.avalon/host/json.js";
#target photoshop
...
var log = new LogFactory('c:\\Program Files (x86)\\Common Files\\Adobe\\CEP\\extensions\\com.avalon\\myLog.log'); // =>; creates the new log factory
                            // as no Folder has been specified in the path the temp folder will be used
log('Hello', 'i', 'happy'); //=> [1] [INFO]     😃Hello [Sun Mar 05 2017 09:57:23 GMT+0200 332ms]
log('Drat', 'e');           //=> [2] [ERROR]    Drafter the shorter loot [Sun Mar 05 2017 09:57:23 GMT+0200 332ms]
log('42', 'I', 'hand');     //=> [3] [IMPORTANT] 👉42 [Sun Mar 05 2017 09:57:23 GMT+0200 332ms]
log('Bye');                 //=> [4] [LOG]      bye [Sun Mar 05 2017 09:57:23 GMT+0200 332ms]
...
function getLayers() {
    ...
    for (var i = count; i >= 1; i--) {
      ...
      ref2.putIndex(charIDToTypeID('Lyr '), i);   // not returning groups eg LayerSets
      ...
      if (layerSection == 'layerSectionStart') { // Group start and end
        group = true;
        groupId = desc.getString(stringIDToTypeID("layerID"));              
      }
      ...
      layer.group = group; // true if group, LayerSet, false for regular ArtLayer
      layer.type = getLayerTypeWithName(layer.name);
      layer.id = desc.getString(stringIDToTypeID("layerID"));
      layer.visible = desc.getBoolean(stringIDToTypeID("visible"));
      log(" name: " + layer.name + " groupId " + layer.groupId + " group " + layer.group); 
...
function saveAs(output_path, ext, as_copy){
    //var saveName = app.activeDocument.fullName + ".jpg";
